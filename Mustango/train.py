...
def parse_args():
    ...
    parser.add_argument(
        "--validation_file", type=str, default="data/metadata_test_indian.json",
        help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--validation_file2", type=str, default="data/metadata_test_indian.json",
        help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--test_file", type=str, default="data/metadata_test_indian.json",
    ...
    parser.add_argument(
        "--num_warmup_steps", type=int, default=1000,
...
def main():
    ...
    ## Freeze the model
    for name, child in model.named_modules():  # Use named_modules for better visibility
        for param in child.parameters():
            param.requires_grad = False

    for name, child in model.named_modules():  # Use named_modules for better visibility
        if isinstance(child, Adapter):  # Check if the module is of type Adapter
            # print(f"Unfreezing Adapter module: {name}")
            for name, param in child.named_parameters():
                # print(f"{name} requires_grad: {param.requires_grad}")
                param.requires_grad = True  # Unfreeze adapter parameters
    ...
    print("% of trainable parameters: ", num_trainable_parameters/sum(p.numel() for p in model.parameters()))
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                with torch.no_grad():
                    ...
                    unwrapped_vae = unwrapped_vae.to(device)
                ...
                if args.model_type=='Tango':
                    ...
                    true_latent = true_latent.cuda()
                    # text = text.cuda()
                    # beats = beats.cuda()
                    # chords = chords.cuda()
                    # chords_time = chords_time.cuda()
                    u_val = torch.unique(true_latent)
                    if len(u_val) <= 1 : 
                        print(u_val)
                    # print()
        ...
        if accelerator.is_main_process:    
            ...
            sparsity = monitor_sparsity(unwrapped_model)
            result["downsample_w"] = sparsity[0][0]
            result["downsample_b"] = sparsity[0][1]
            result["midsample_w"] = sparsity[1][0]
            result["midsample_b"] = sparsity[1][1]
            result["upsample_w"] = sparsity[2][0]
            result["upsample_b"] = sparsity[2][1]
...
def monitor_sparsity(model):
    """
    Monitor the sparsity (percentage of near-zero weights) of adapter layers.
    """
    down_sparsity_weight = []
    down_sparsity_bias = []
    middle_sparsity_weight = []
    middle_sparsity_bias = []
    up_sparsity_weight = []
    up_sparsity_bias = []
    
    for name, param in model.named_parameters():
        if "adapter" in name and "down" in name and param.requires_grad:
            if "weight" in name :     
                down_sparsity_weight.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
            # print(f"Layer: {name}, Sparsity: {sparsity:.2f}%")
            
            if "bias" in name : 
                down_sparsity_bias.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
            
        if "adapter" in name and "mid" in name and param.requires_grad:
            if "weight" in name :     
                middle_sparsity_weight.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
            # print(f"Layer: {name}, Sparsity: {sparsity:.2f}%")
            
            if "bias" in name : 
                middle_sparsity_bias.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
        
        if "adapter" in name and "up" in name and param.requires_grad:
            if "weight" in name :     
                up_sparsity_weight.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
            # print(f"Layer: {name}, Sparsity: {sparsity:.2f}%")
            
            if "bias" in name : 
                up_sparsity_bias.append((torch.abs(param) < 1e-3).float().mean().item() * 100)
        
    return ((down_sparsity_weight, down_sparsity_bias), (middle_sparsity_weight, middle_sparsity_bias) , (up_sparsity_weight, up_sparsity_bias))
