...
class text2music_feature_generator():
	def __init__(self, mode, beat_generator = None, chord_generator = None, max_dur = 30):
	...
	def extract_chords(self, chord_string):
		...
		return chords
	def get_rule_based_beat(self, text, default_beat_num = 4, default_bpm = 70, default_duration = 30):
...
def main():
	...
	if train_args.hf_model:
		...
		vae, stft = vae.to(device), stft.to(device) #Nic, uncomment when GPU avail
		mustango = Mustango(device=device)
		model = mustango.model
	...
	if not args.baseline : 
		print("Inference from fine-tuned model")
	...
	def audio_text_matching(waveforms, text, sample_freq=16000, max_len_in_seconds=30):
		new_freq = 48000
		resampled = []
	...
	output_file_names = [json.loads(line)["location"] for line in open(args.test_file).readlines()]
	...
	for k in tqdm(range(0, len(text_prompts), batch_size)):
		...
		beat = [txt for txt in text]
		chord = [txt for txt in text]
		chord_timing = [txt for txt in text]
		...
		# with torch.no_grad():
		# 	latents = model.inference(text, beat, chord, chord_timing, scheduler, device, num_steps, guidance, num_samples, disable_progress=True) #TODO, nic change
	...
	if num_samples == 1:
		...
		output_dir = "outputs/out_1732794477_1732727096_best_steps_200_guidance_3.0"
		os.makedirs(output_dir, exist_ok=True)
		# print(output_file_names)
		for j, wav in enumerate(all_outputs):
			sf.write("{}/{}".format(output_dir,output_file_names[j]), wav, samplerate=16000)
		if args.evaluate:
			# output_dir = "outputs/out_1728665577_1727867905_best_steps_200_guidance_3.0"
			...
			result["inference_prompts"] = args.test_file
			wandb.log(result)
