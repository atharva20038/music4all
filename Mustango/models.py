...
class MusicAudioDiffusion(nn.Module):
	def __init__(
		...
		print("Music Audio Diffusion Model")
		
		if unet_model_config_path:
			print("UNet2DConditionModelMusic.load_config")
			print(unet_model_config_path)
			...
			## Add adapter to UNet2DConditionModel 
	...
	def forward(self, latents, prompt, beats, chords,chords_time, validation_mode=False):
		...
		# with torch.no_grad():
		encoded_beats, beat_mask = self.encode_beats(beats, device) #batch, len_beats, dim; batch, len_beats
		encoded_chords, chord_mask = self.encode_chords(chords,chords_time, device)
	...
	@torch.no_grad()
	def inference(self, prompt, beats, chords,chords_time, inference_scheduler, device, num_steps=20, guidance_scale=3, num_samples_per_prompt=1, 
				  disable_progress=True):
		device = self.text_encoder.device
		classifier_free_guidance = guidance_scale > 1.0
		batch_size = len(prompt) * num_samples_per_prompt
		...
		if classifier_free_guidance:
			prompt_embeds, boolean_prompt_mask = self.encode_text_classifier_free(prompt, num_samples_per_prompt)
			encoded_beats, beat_mask = self.encode_beats_classifier_free(beats, num_samples_per_prompt, device) #batch, len_beats, dim; batch, len_beats
			encoded_chords, chord_mask = self.encode_chords_classifier_free(chords, chords_time, num_samples_per_prompt, device)
		else:
			prompt_embeds, boolean_prompt_mask = self.encode_text(prompt)
			prompt_embeds = prompt_embeds.repeat_interleave(num_samples_per_prompt, 0)
			boolean_prompt_mask = boolean_prompt_mask.repeat_interleave(num_samples_per_prompt, 0)
	...
	def encode_beats_classifier_free(self, beats, num_samples_per_prompt, device):
		with torch.no_grad():
			out_beat = []
			out_beat_timing = []
			out_mask = []
	...
	def encode_chords_classifier_free(self, chords, chords_time, num_samples_per_prompt, device):
