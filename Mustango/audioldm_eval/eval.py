...
from audioldm_eval.metrics.mmd import CMMD
...
class EvaluationHelper:
    ...
    def get_filename_intersection_ratio(
        ...
        print(len(intersect_keys) / len(keyset1))
    ...
    def calculate_lsd(self, pairedloader, same_name=True, time_offset=160 * 7):
        ...
        for _, _, filename, (audio1, audio2) in tqdm(pairedloader, leave=False):
            ...
            if audio1.numel() == 0 or audio2.numel() == 0:
                print(f"Empty audio detected: {filename}")
                continue
            ...
            # Check for empty slices before computing mean
            if audio1.size == 0 or audio2.size == 0:
                print(f"Empty slice after processing: {filename}")
            ...
            if audio1.size == 0 or audio2.size == 0:
                print(f"Empty slice after slicing")
                continue
        ...
        print("The mean warning is below")
    ...
    def calculate_metrics(self, generate_files_path, groundtruth_path, same_name, limit_num=None):
        ...
        cmmd_score = self.cmmd.score(generate_files_path, groundtruth_path, limit_num=limit_num)
